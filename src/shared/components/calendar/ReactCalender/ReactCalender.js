...
const downloadUrl = apiBaseUrl + "/api/calendar-events/generate/csv"

let date = ""
const localizer = momentLocalizer(moment);
localizer.formats.yearHeaderFormat = 'YYYY'
let eventResultsDataArray = [];
const getAllEvents = async (startDate, enddate) => await getEventData(startDate, enddate).then(res => res);
...
const ReactCalender = () => {
  ...
  const toggle = (e) => {
    let resultsArray = []
    resultsArray.push(e)
    console.log(resultsArray, !open);
    setmodalData(resultsArray)
    return setOpen(!open)
  };
...
class CalendarToolbar extends React.Component {
  ...
  generateCsvUrl = () => {
    ...
    return `${downloadUrl}?startDate=${moment(startDate).startOf(this.props.view).valueOf()}&endDate=${moment(endDate).endOf(this.props.view).valueOf()}`
  }
    ...
    return (
      <div className="customeToolbar">
        ...
        <div style={{ display: "flex" }}>
          <Button style={{ height: '38px', marginRight: '10px', color: '#fff' }} className='bg-color-primary fw-600 rounded-0 border-0' size="lg">
          <a href={this.generateCsvUrl()} style={{ textDecoration: "none", color: '#fff' }}>Download</a></Button>
